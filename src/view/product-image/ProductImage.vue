...
const sendMessage = async () =>{
  ...
  if(getTokenCode.value===500){
    ...
    if(input.value===""){
      ...
      if(firstTime){
        const wordToImageDiv = document.getElementById('wordToImageDiv');
        wordToImageDiv.classList.add('orContentDivHidden');
        firstTime = false
      }
      ElMessage({
        message: '小卓正在思考...请稍等',
      ...
      messagelistData.value.push(base64ImageData)
      input.value= '';
    }
  }
}
const {  nickname ,base64ImageData} = storeToRefs(useXiaozhuoStore())
hljs.highlightAll();
onMounted(async () => {
...
<template>
  ...
  <div class="scrolldiv">
    <el-scrollbar>
      <div id="wordToImageDiv" class="orContentDiv"  v-if="messagelistData.length === 0">
        <p class="welcometext">输入文字，图像随心生成。</p>
